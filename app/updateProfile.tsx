...
export default function UpdateProfile() {
  ...
  const handleSubmit = async (email: string) => {
    ...
    if (
      newDetail.Role !== undefined &&
      newDetail.Role !== (user?.user_type || "")
    ) {
    ...
    if (
      newDetail.Email !== undefined &&
      newDetail.Email !== (user?.email || "")
    ) {
      updatedFields.email = newDetail.Email;
    }
    if (
      newDetail.location !== undefined &&
      newDetail.location !== (user?.location || "")
    ) {
    ...
    if (user?.user_type?.toLowerCase() === "mentor" && userUpdateSuccess) {
      ...
      const { error: mentorError } = await supabase
        ...
        .update({
          ...
          availability_hours_per_week:
            mentorProfile.availability_hours_per_week,
      ...
      if (mentorError) {
        ...
        Alert.alert(
          "Error",
          `Failed to update mentor profile: ${mentorError.message}`
        );
